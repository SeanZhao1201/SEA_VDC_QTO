...
- References RhinoCommon 7.28 (Rhino 7). No version gating exists in code; per McNeel guidance the plugin also loads in Rhino 8 on Windows (Rhino 8 defaults to .NET Core; the xBIM/binding-redirect stack is the most fragile part there — `SetDotNetRuntime` > NETFramework is the fallback). Rhino 8 for Mac cannot load it (WPF/COM).
...
**IFC export** (`IFCMethods.cs`, xBIM 5.1): builds an in-memory **IFC4-only** model with the spatial hierarchy `IfcProject` → `IfcSite` ("Default Site") → `IfcBuilding` ("Concrete Building") → one `IfcBuildingStorey` per floor. Storeys are created by `CreateBuildingStoreys` from `ElevationInput.floorElevations` (storey `Elevation` in millimetres, ordered by elevation; duplicate floor names collapse to one storey at the lowest elevation) plus a shared "Unassigned" fallback storey for floor buckets with no elevation entry (e.g. `"-"`). `CreateAndAddIFCElement` routes each floor bucket to its storey via `storey.AddElement`. Floors are also still written as a `"FLOOR"` text property inside the `"QTO Properties"` pset (Rhino attribute user strings go into a separate `"QTO Attributes"` pset). Geometry is exported as tessellated `IfcFaceBasedSurfaceModel` meshes in absolute world coordinates converted to millimetres (`GetModelUnitToMillimeterFactor`, via `RhinoMath.UnitScale` so every Rhino unit system is handled) — storey placements carry the elevation but element placements are deliberately not chained to them.
