# -*- coding: utf-8 -*-
"""Batch run of sideform_gen_rhino on the currently open model, EXPORT mode.

Launched as:  Rhino.exe <model.3dm> /nosplash
              /runscript="-_RunPythonScript <this file>"
Prefer opening the pour-break DERIVED model (the split pieces carry POUR
user strings, so joints get bulkheads with pour ownership). The document
is never modified and never saved. Output lands in the staging folder:
sideforms_out.3dm + sideforms_out.json + sideform_model_log.txt
(+ sideform_model_error.txt on crash).
"""
...
try:
    ...
    n_pour = sum(1 for o in doc.Objects
                 if o is not None and o.Attributes is not None
                 and o.Attributes.GetUserString("POUR"))
    pre("objects with POUR user strings: {0}".format(n_pour))

    sf.main({
        "mode": "export",
        "export_path": os.path.join(STAGE, "sideforms_out.3dm"),
        "log_path": os.path.join(STAGE, "sideform_model_log.txt"),
    })
    log_path = os.path.join(STAGE, "sideform_model_log.txt")
    try:
    ...
    with io.open(os.path.join(STAGE, "sideform_model_error.txt"), "w",
